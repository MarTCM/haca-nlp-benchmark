...
- **Français** — `xlm-sentiment` ★ (`cardiffnlp/twitter-xlm-roberta-base-sentiment`, off-the-shelf
  3-class) is the default: on the 90-utterance gold all French configs land within noise (0.43–0.49)
  and the synthetic fine-tunes don't reliably beat it (see `FINETUNING.md` §6), so we default to the
